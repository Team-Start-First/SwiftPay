     ...
     <Modal isOpen={isOpen} onClose={resetAndClose} title="Request money">
      {link ? (
        <div className="space-y-4">
          <p className="text-sm text-slate-600 dark:text-slate-400">Share this link to request payment:</p>
          <div className="flex gap-2">
            <input
              readOnly
              value={link}
              className="flex-1 min-w-0 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl px-3 py-2.5 text-sm text-slate-800 dark:text-slate-100"
            />
            <button
              type="button"
              onClick={handleCopy}
          ...
          <button
            ...
            className="w-full py-3 rounded-xl font-semibold text-slate-700 dark:text-slate-300 bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 transition-colors"
          >
            Close
          </button>
        </div>
        ...
        <form onSubmit={handleGenerate} className="space-y-4">
          <div>
            <label htmlFor="rq-amount" className="text-xs font-semibold text-slate-600 dark:text-slate-400">
              Amount requested
            </label>
            <input
              id="rq-amount"
          ...
          <div>
            <label htmlFor="rq-note" className="text-xs font-semibold text-slate-600 dark:text-slate-400">
              Note (optional)
            </label>
            <input
              id="rq-note"
