  ...
  return (
    <div className="relative" ref={containerRef}>
      <button
        ...
        aria-haspopup="menu"
        aria-expanded={open}
        className="flex items-center gap-2 pl-2 pr-3 py-1.5 rounded-full bg-white/50 backdrop-blur-xl border border-white/40 shadow-sm"
      >
        {avatarUrl ? (
        ...
        <span className="text-sm font-semibold text-slate-700 max-w-[120px] truncate">{name}</span>
        <FiChevronDown size={14} className={`text-slate-500 transition-transform ${open ? "rotate-180" : ""}`} />
      </button>
      ...
      <AnimatePresence>
        {open && (
          <motion.div
            ...
            role="menu"
            className="absolute right-0 mt-2 w-56 rounded-2xl bg-white/95 backdrop-blur-lg border border-white/60 shadow-xl overflow-hidden z-40"
          >
