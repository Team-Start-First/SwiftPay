...
const BalanceCard = ({ totalBalance = "$12,480.50", monthlyChange = "+$1,240", wallets = [] }) => {
  return (
    <div className="relative overflow-hidden rounded-3xl border border-white/60 dark:border-slate-700 bg-gradient-to-br from-blue-100 via-pink-100 to-pink-50 dark:from-slate-800 dark:via-slate-800 dark:to-slate-900 p-6 sm:p-9">
      ...
      <div className="relative z-10 flex flex-col sm:flex-row sm:items-start sm:justify-between gap-6">
        <div>
          <div className="flex items-center gap-2 text-sm font-semibold text-slate-600 dark:text-slate-400 mb-2">
            <FiTrendingUp className="text-slate-500 dark:text-slate-400" />
            Total balance
          </div>
          <div className="text-4xl sm:text-5xl font-bold tracking-tight text-slate-900 dark:text-white tabular-nums">
            {totalBalance}
          </div>
        ...
        <div className="flex items-center gap-3 rounded-2xl bg-white/60 dark:bg-slate-900/60 backdrop-blur-lg border border-white/60 shadow-md px-4 py-3 self-start animate-[float_5s_ease-in-out_infinite]">
          ...
          <div className="text-xs text-slate-700 dark:text-slate-300 leading-tight">
            Last transfer
            <div className="text-base font-bold text-slate-900 dark:text-white">2.3s</div>
          </div>
        </div>
      </div>
      ...
      <div className="relative z-10 mt-7 grid grid-cols-2 sm:grid-cols-4 gap-3">
        {wallets.map((wallet) => (
          <div
            key={wallet.code}
            className="rounded-2xl bg-white/55 dark:bg-slate-900/55 backdrop-blur-md border border-white/60 px-4 py-3"
          >
            <div className="text-xs font-bold text-slate-600 dark:text-slate-400 tracking-wide">{wallet.code}</div>
            <div className="text-lg font-bold text-slate-900 dark:text-white tabular-nums mt-1">{wallet.amount}</div>
          </div>
        ))}
      </div>
    </div>
